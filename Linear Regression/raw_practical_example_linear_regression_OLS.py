...
# if we have n categories for a feature, we hace to create n-1 dummies
...
# %%
reg.score(x_train, y_train) #0.745 - relatively good result (our model it's explaining 75% of variability our data)
# %%
...
data_cleaned['Brand'].unique() # audo dropped one (nie ma go) - audi is the benchmark
...
#pd.options.display.max_rows = 999 # all rows to display
#pd.set_option('display.float_format', lambda x: '%.2f' % x) # zaokraglenie do 0.00
df_pf.sort_values(by=['Difference_%'])
